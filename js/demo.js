...
$(document).ready(function (e) {
    ...
    function showTab(thisDiv) {
        ...
        if (typeof funRouteMap !== 'undefined') {
            funRouteMap.invalidateSize(false);
        }
    }
    ...
    //
    let defaultKey = "2e1be208-6fa8-410b-a109-713fcfa2db5e";
    ...
    let ghFunRoute = new GraphHopper.FunRoute({key: defaultKey, host: host});
...
//    }
    ...
    let funRouteMap = createMap('fun-route-map');
    setupFunRouteAPI(funRouteMap, ghFunRoute);
...
function setupFunRouteAPI(map, funRouteClient) {
    map.setView([52.521235, 13.3992], 12);
    ...
    // 更新距离显示
    $('#target_distance').on('input', function() {
        $('#distance_display').text($(this).val() + ' km');
    });
    
    // 更新方向显示
    $('#start_bearing').on('input', function() {
        $('#bearing_display').text($(this).val() + '°');
    });
    
    // 路线类型切换
    $('#route_type_select').on('change', function() {
        if ($(this).val() === 'roundtrip') {
            $('#roundtrip_controls').show();
        } else {
            $('#roundtrip_controls').hide();
        }
    });
    
    // 地图点击事件
    map.on('click', function(e) {
        let routeType = $('#route_type_select').val();
        
        if (routeType === 'roundtrip') {
            // 环形路线只需要一个起点
            markersLayer.clearLayers();
            startPoint = [e.latlng.lng, e.latlng.lat]; // GraphHopper API期望[lng, lat]格式
            L.marker([e.latlng.lat, e.latlng.lng], {icon: iconObject}).addTo(markersLayer);
            endPoint = null;
        } else {
            // 点对点路线需要两个点
            if (!startPoint) {
                ...
                endPoint = [e.latlng.lng, e.latlng.lat]; // GraphHopper API期望[lng, lat]格式
                let endIcon = L.icon({
                    iconUrl: './img/marker-icon.png',
                    shadowSize: [50, 64],
                    shadowAnchor: [4, 62],
                ...
                L.marker([e.latlng.lat, e.latlng.lng], {icon: endIcon}).addTo(markersLayer);
            } else {
                // 重新开始
                markersLayer.clearLayers();
                startPoint = [e.latlng.lng, e.latlng.lat]; // GraphHopper API期望[lng, lat]格式
    ...
    // 生成路线按钮
    $('#generate_fun_route').click(function() {
        if (!startPoint) {
            $('#fun-route-error').text('请先在地图上点击设置起点');
            return;
        }
        
        let routeType = $('#route_type_select').val();
        if (routeType === 'point_to_point' && !endPoint) {
            $('#fun-route-error').text('点对点路线需要设置终点');
            return;
        }
        
        $('#fun-route-error').text('');
        $('#fun-route-response').html('正在生成路线...');
        
        let params = {
            start_point: startPoint,
            target_distance: parseInt($('#target_distance').val()) * 1000, // 转换为米
            curve_level: $('#curve_level_select').val(),
            route_type: routeType,
            profile: 'car', // GraphHopper API标准profile参数
            instructions: true,
        ...
        if (routeType === 'roundtrip') {
            params.start_bearing = parseInt($('#start_bearing').val());
        } else {
            params.end_point = endPoint;
        }
        
        funRouteClient.generateFunRoute(params).then(function(result) {
            // 清除之前的路线
            if (routeLayer) {
            ...
            if (result.paths && result.paths.length > 0) {
                ...
                if (path.points && path.points.coordinates) {
                    let coordinates = path.points.coordinates.map(point => [point[1], point[0]]); // 转换为leaflet格式
            ...
            if (result.paths && result.paths.length > 0) {
                let path = result.paths[0];
                info += '<p>总距离: ' + (path.distance / 1000).toFixed(2) + ' km</p>';
                info += '<p>预计时间: ' + Math.round(path.time / 60000) + ' 分钟</p>';
            } else {
                info += '<p>无法获取路线信息</p>';
            }
            info += '<p>路线类型: ' + (routeType === 'roundtrip' ? '环形路线' : '点对点路线') + '</p>';
            info += '<p>弯道等级: ' + $('#curve_level_select option:selected').text() + '</p>';
            $('#fun-route-response').html(info);
            $('#fun-route-error').text('');
        }).catch(function(error) {
            $('#fun-route-error').text('生成路线失败: ' + error.message);
            $('#fun-route-response').html('');
        });
    });
    
    // 清除路线按钮
    $('#clear_fun_route').click(function() {
        markersLayer.clearLayers();
        if (routeLayer) {
            map.removeLayer(routeLayer);
            routeLayer = null;
